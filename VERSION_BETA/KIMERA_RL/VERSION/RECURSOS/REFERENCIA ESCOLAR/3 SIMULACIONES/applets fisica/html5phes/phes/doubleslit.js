...
// Sprachabhängige Texte sind in einer eigenen Datei (zum Beispiel doubleslit_de.js) abgespeichert.

// Farben:

var colorBackground1 = "#404040";                          // Hintergrundfarbe Versuchsaufbau
var colorBackground2 = "#ffff00";                          // Hintergrundfarbe Versuchsergebnis
var colorDoubleSlit = "#808080";                           // Farbe für Doppelspalt
...
function start () {
  ...
  getElement("ip2a",text02);                               // Erklärender Text (Spaltabstand)
  ...
  alpha = 17.5*DEG;                                        // Startwert Winkel (Bogenmaß)
...
function addNewOption (ch, w, k) {
  ...
  s += " ("+symbolOrder+" = "+k+")";                       // Ordnung in Klammer hinzufügen
...
function reactionSlider1 () {
  lambda = 1e-8*(38+Number(sl1.value));                    // Wellenlänge (m)
  updateInput(true,false,false);                           // Eingabefeld aktualisieren
  reaction(true);                                          // Daten übernehmen, rechnen, Ausgabe, neu zeichnen
  }
  
// Reaktion auf Schieberegler für Spaltabstand:
// Seiteneffekt lambda, d, alpha
...
function reactionSelect1 () {
  ...
  alpha = Math.asin(k*lambda/d);                           // Winkel (Bogenmaß)
  updateInput(false,false,true);                           // Eingabefeld aktualisieren
...
function reactionSelect2 () {
  ...
  alpha = Math.asin((k+0.5)*lambda/d);                     // Winkel (Bogenmaß)
  updateInput(false,false,true);                           // Eingabefeld aktualisieren
  reaction(false);                                         // Daten übernehmen, rechnen, Ausgabe, neu zeichnen
  }
...
// i2 ... Flag für Aktualisierung des Spaltabstands
...
function updateMaxMin () {
  ...
  for (var k=0; k<=maxK; k++)                              // Für alle Maxima ...
    addNewOption(ch1,Math.asin(k*lambda/d),k);             // Neuen Eintrag zur Liste hinzufügen
  ...
  for (k=1; k<=maxK; k++)                                  // Für alle Minima ... 
    addNewOption(ch2,Math.asin((k-0.5)*lambda/d),k);       // Neuen Eintrag zur Liste hinzufügen
...
function updateOutput () {
  op.innerHTML = ToString(intensity(alpha),3,true);        // Relative Intensität
  }
...
function setPolygons () {
  poly1 = new Array(4);                                    // Parallelogramm für Doppelspalt
...
// Berechnung der (relativen) Intensität:
// x ... Winkel (Bogenmaß)
// Rückgabewert zwischen 0 und 1
  
function intensity (x) {
  var c = d*2*Math.PI/lambda;                              // Hilfsgröße
  return (1+Math.cos(c*Math.sin(x)))/2;                    // Rückgabewert
  }
...
// Strahlen für die Maxima (nach dem Doppelspalt):
  	
function raysMaxima () {
  var maxK = Math.floor(d/lambda);                         // Maximale Ordnung eines Maximums
  for (var k=-maxK; k<=maxK; k++) {                        // Für alle Maxima (links und rechts) ...
  	var w = Math.asin(k*lambda/d);                         // Winkel (Bogenmaß)
  	var x = RAD*Math.cos(w), y = RAD*Math.sin(w);          // Räumliche Koordinaten
  	var u = screenU(x,y), v = screenV(x,y,0);              // Zugehörige Bildschirmkoordinaten (Pixel)
...
function doubleSlit () {
  drawPolygon(poly1,colorDoubleSlit,true);                 // Ausgefülltes Parallelogramm mit Rand
  var f = 2e-6;                                            // Hilfsgröße
  var u0 = Math.max(screenU(0,-d/f),uM+1);                 // Waagrechte Bildschirmkoordinate für rechten Spalt
  var v0 = screenV(0,-d/f,-20);                            // Senkrechte Bildschirmkoordinate für rechten Spalt (unten)
  var v1 = screenV(0,-d/f,20);   	                       // Senkrechte Bildschirmkoordinate für rechten Spalt (oben) 
  line(u0,v0,u0,v1);                                       // Rechter Spalt
  u0 = Math.min(screenU(0,d/f),uM-1);                      // Waagrechte Bildschirmkoordinate für linken Spalt
  v0 = screenV(0,d/f,-20);                                 // Senkrechte Bildschirmkoordinate für linken Spalt (unten)
  v1 = screenV(0,d/f,20);  	                               // Senkrechte Bildschirmkoordinate für linken Spalt (oben)
